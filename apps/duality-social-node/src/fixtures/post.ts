...
import { DefaultReactionsTypeEnum, IPost, IPostDocument } from '@duality-social/duality-social-lib';
...
export const makePost = (overrides = {}): Partial<IPostDocument> => {
    const post = {
        ...
        reqTransLangs: [faker.helpers.arrayElement(['en', 'es', 'fr', 'de', 'it', 'pt', 'ru', 'zh', 'ja', 'ko'])],
        aiReqTransLangs: [faker.helpers.arrayElement(['en', 'es', 'fr', 'de', 'it', 'pt', 'ru', 'zh', 'ja', 'ko'])],
        ...
        metadata: {
          ...
          reactionsByType: Object.fromEntries(Object.values(DefaultReactionsTypeEnum).map(type => [type, faker.number.int({ min: 0, max: 100 })])),
